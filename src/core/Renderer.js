...
export class Renderer {
	...
	constructor( { outputFormat = GPU.format } = {} ) {
		...
		this.rt = new RenderTarget( 1, 1, { colors: SCENE_COLORS, depth: 'depth32float', label: 'scene' } );
		...
		this.postPass = new FullscreenPass( {
			...
			code: /* wgsl */`
				...
				fn fragment( in: FSIn ) -> vec4f {
					...
					let dist = viewDepth( max( d, 1e-7 ) );
					let haze = ( 1.0 - exp( - max( dist - 50.0, 0.0 ) * 0.0014 ) ) * step( dist, 1300.0 );
					c = mix( c, frame.horizonColor * 2.2, haze * 0.5 );
					c *= frame.exposure;
					c = aces( c );
					let l = luminance( c );
					c = mix( vec3f( l ), c, post.saturation );
					c = c * vec3f( 1.02, 1.0, 0.96 );
					let q = in.uv - 0.5;
		...
		// a warm afternoon
		G.sunDir.value.set( - 0.45, 0.62, 0.55 ).normalize();
		G.sunColor.value.setRGB( 3.3, 3.05, 2.7 );
		G.skyIrradiance.value.setRGB( 0.32, 0.4, 0.52 );
		G.horizonColor.value.setRGB( 0.62, 0.66, 0.72 );
		G.exposure.value = 0.95;
	...
	render( scene, camera, outputView ) {
		...
		this.meshRenderer.render( scene, {
			camera, kind: 'main', colorViews: this.rt.textures.map( ( t ) => t.view() ), colorFormats: SCENE_COLORS,
